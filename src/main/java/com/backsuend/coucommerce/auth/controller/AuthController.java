...
public class AuthController {
	...
	@PostMapping("/signup")
	public ResponseEntity<ApiResponse<Void>> signup(@Valid @RequestBody SignupRequest memberRequestDto) {
		authService.signup(memberRequestDto);
		return ResponseEntity.ok(ApiResponse.success("회원가입이 완료되었습니다."));
	}

	@PostMapping("/login")
	public ResponseEntity<ApiResponse<TokenDto>> login(@Valid @RequestBody LoginRequest loginRequestDto) {
	...
	public ResponseEntity<ApiResponse<TokenDto>> reissue(@RequestBody TokenRequest tokenRequestDto) {
		return ResponseEntity.ok(ApiResponse.success(authService.reissue(tokenRequestDto), "토큰이 재발급되었습니다."));
	}
